  ...
  for (const i in ob) {
    ...
    // Else store ob[i] in result directly
      ...
      if (Array.isArray(ob[i])) {
        let items = {};
        if (typeof ob[i][0] === "object" && !Array.isArray(ob[i][0])) {
          const reducedObj = ob[i].reduce((result, acc) => {
            return {
              ...acc,
              ...result,
            };
          }, {});
          const temp = parseSchema(reducedObj);
          items = temp;
        } else {
          items = {
            type: Array.isArray(ob[i][0]) ? "array" : typeof ob[i][0],
            example: ob[i][0],
          };
        }
        result[i] = {
